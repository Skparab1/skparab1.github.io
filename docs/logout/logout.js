...
function draw(){
  ...
  if (catalog && false){
    ...
    while (c >= 0){
      fill(0,0,c);
      stroke(0,0,c);
      rect(0,255-c,windowWidth,1);
      c -= 1;
    }
    
    fill(225,0,0);
    text('Skparab1 Redirect catalog',100,100);
    
    let z = 0;
    let ypos = 200;
    let xpos = 50;
    while (z < keyword.length){
      textSize(50);
      fill(200,100,0);
      stroke(200,200,0);
      text(keyword[z],xpos,ypos);
      
      if (clickpos[0] > xpos-10 && clickpos[0] < xpos+360 && clickpos[1] > ypos-10 && clickpos[1] < xpos+110){
        openwindow = url[z];
